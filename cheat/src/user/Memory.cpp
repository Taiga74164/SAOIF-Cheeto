...
uintptr_t MemoryResolver::RelativeJMP() const
{
	BYTE* code = reinterpret_cast<BYTE*>(address);
	if (code[0] != 0xE9 && code[0] != 0xE8)
		return address;
	return address + *reinterpret_cast<int32_t*>(&code[1]) + 5;
}

uintptr_t MemoryResolver::RelativeMOV32() const
{
	BYTE* code = reinterpret_cast<BYTE*>(address);
	if (code[0] != 0x8b || (code[1] != 0x05 && code[1] != 0x0D && code[1] != 0x15 && code[1] != 0x1D))
		return address;
	return address + *reinterpret_cast<int32_t*>(&code[2]) + 6;
}

uintptr_t MemoryResolver::RelativeMOV() const
{
	BYTE* code = reinterpret_cast<BYTE*>(address);
	if (code[0] != 0x48 || (code[2] != 0x05 && code[2] != 0x0D && code[2] != 0x15 && code[2] != 0x1D))
		return address;
	return address + *reinterpret_cast<int32_t*>(&code[3]) + 7;
}

uintptr_t MemoryResolver::RelativeXMM128() const
{
	BYTE* code = reinterpret_cast<BYTE*>(address);
	if (code[1] != 0x0F || (code[2] != 0x10 && code[2] != 0x11))
		return address;
	return address + *reinterpret_cast<int32_t*>(&code[4]) + 8;
}

uintptr_t MemoryResolver::JMP() const
{
...
uintptr_t MemoryResolver::MOV32() const
{
	uintptr_t addr = RelativeMOV32();
	addr -= module;
	return addr + module;
}

uintptr_t MemoryResolver::MOV() const
{
...
uintptr_t MemoryResolver::XMM128() const
{
	uintptr_t addr = RelativeXMM128();
	addr -= module;
	return addr + module;
}
